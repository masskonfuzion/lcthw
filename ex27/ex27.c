...
/* 
 * A safer version that checks for many common errors using the length of each string to control the loops and termination.
 *
 */
int safercopy(int from_len, char *from, int to_len, char *to)
{
	// Why can we say "&& from and to can't be NULL"
	assert(from != NULL && to != NULL && "from and to can't be NULL");

	int i = 0;

	// Take the max len
	// (the funky notation is stupid C if/then shorthand
	// it says: if from_len > to_len - 1, max = to_len; else, max = from_len
